...
async function loadNotice(){
  try{
    ...
    const latest = xml.querySelector('notice');
    const title = latest?.querySelector('title')?.textContent?.trim();
    const body = latest?.querySelector('body')?.textContent?.trim();
    $('#noticeText').textContent = title && body ? `${title}: ${body}` : 'اطلاعیه فعالی ثبت نشده است.';
  }catch{ $('#noticeText').textContent = 'اطلاعیه فعالی ثبت نشده است؛ برای خبرهای روز از واتساپ یا اینستاگرام با ما در ارتباط باشید.'; }
}
loadNotice();
